...
my @glim = openFile($opts{g}); #opening glimmer file
...
sub readGlimmer{
	...
	#glimmer is the worst and ruins everything
	my $line; #each line string
	my $gene_num=0; #gene counter
	my $seq_num=1; #contig counter
	my @values; #values of each line
	...
	foreach $line(@glim){	#going through the lines of the Glimmer file
		chomp $line;	#removing newline character from each line
		$gene_num += 1;
		@values = split("\t", $line);
		#$contig_id = $values[0];
		$contig_id = substr $values[8], 6, 5; #instead storing the ORF number
		$start = $values[4];
		$stop = $values[3];
		$glim_genes{$gene_num}{"contig_id"} = $contig_id; #but is useless with glimmer
		#getting the contig number
		if ($gene_num == 1 || $glim_genes{$gene_num}{"contig_id"} > $glim_genes{$gene_num-1}{"contig_id"}) { #if it's the first gene or if the ORF is larger than it's predecessor
			$glim_genes{$gene_num}{"seq_num"} = $seq_num;
		}
		else{ #if the gene ID doesn't equal the one before it then increase the sequence number
			$seq_num += 1;
			$glim_genes{$gene_num}{"seq_num"} = $seq_num;
		}
		$glim_genes{$gene_num}{"start"} = $start;	#this stores the start value for the 1st gene (and then so on..) found by Prodigal
		$glim_genes{$gene_num}{"stop"} = $stop;	#same with stop position
	}
}
...
sub compareGFF{
	...
	if ($size1 >= $size2) {
		#compare the sequence number, the start and the stop
		#if it's a match, add to a count
		#also add to a list to see the overlap with the third, and save the info to output file
	}
}
...
readHMM();
readGlimmer();
compareGFF(%prod_genes, %glim_genes);
...
=put
Things to do:
First: write the compareGFF subroutine to do 2 way comparisons between all 3 methods
Second: Get the matches and see if they are the same across all three
Third: print the counts to the terminal and print the all 3 overlap to a file (maybe just the info not the whole GFF line - this is just for testing)
