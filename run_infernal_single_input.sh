...
printf "\n[infernal]running infernal cmscan on $1...\n"

# trash default report and output table format
cmscan --fmt 2 --tblout ncRNA_output/$1.tblout /projects/data/team1_GenePrediction/bin/infernal-1.1.2/cm/RF01400.cm $1 > trash.cmscan
rm trash.cmscan

printf "\n[infernal]finish infernal cmscan on $1\n"
printf "\n[infernal]converting output to gff3...\n"

awk '{if (substr($0, 0, 1) != "#" && $19 == "!" ) printf "%s\t%s\t%s\t%d\t%d\t%.5f\t%s\t%s\ttarget name=%s, accession=%s\n",$4,"Infernal-1.1.2","sRNA",$10,$11,$18,$12,".",$2,$3}' ncRNA_output/$1.tblout  > ncRNA_output/$1.temp
awk '{ if( $4 > $5) {t=$5; $5=$4; $4=t; print} else {print} }' ncRNA_output/$1.temp > ncRNA_output/$1.infernal.gff
sed -i 's/ /\t/g' "ncRNA_output/$1.infernal.gff"

rm ncRNA_output/$1.tblout
rm ncRNA_output/$1.temp
