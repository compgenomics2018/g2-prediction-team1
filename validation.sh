...
intersect () {
	...
	do
	bedtools intersect -f 0.80 -r -s -wa -a ${array1[$j]} -b ${array2[$j]}> "intersect_${array3[$j]}.gff"  ##find intersection of the results between 2 tools with 99% coverage using bedtools  	
	done
}
...
coverage () {  ##calculate the coverage of each tool in order to find the best tool
	...
	for ((j=0;j<=$(($i-1));j+=1))
	do
	bedtools intersect -f 0.80 -r -s -a ${array1[$j]} -b ${array2[$j]} > "overlap_${array1[$j]}_${array2[$j]}"
