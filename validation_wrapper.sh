...
intersect () {
	bedtools intersect -f 0.99 -r -a $1 -b $2 > "intersect.gff"  ##find intersection of the results between 2 tools with 99% coverage using bedtools  	
}
...
coverage () {  ##calculate the coverage of each tool in order to find the best tool
	bedtools intersect -f 0.99 -r -a $1 -b $2 > "overlap_$1_$2"
